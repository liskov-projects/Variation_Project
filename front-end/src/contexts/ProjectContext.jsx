...
export const ProjectProvider = ({ children }) => {
  ...
  const addVariation = async (projectId, variationData) => {
    ...
    try {
      const token = await getToken();
      const response = await axios.post(
        `${API_BASE_URL}/api/projects/${projectId}/variations`,
        variationData,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const updateVariation = async (projectId, variationId, variationData) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/api/projects/${projectId}/variations/${variationId}`,
        variationData,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      const errorMessage =
        err.response?.data?.message || err.message || "Failed to update variation";
      setError(errorMessage);
      return { success: false, error: errorMessage };
    } finally {
      setLoading(false);
  ...
  const deleteVariation = async (projectId, variationId) => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message || err.message || "Failed to delete variation";
      setError(errorMessage);
      return { success: false, error: errorMessage };
    } finally {
      setLoading(false);
