...
const runMigration = async () => {
  try {
    ...
    for (const project of verifyProjects) {
      for (const variation of project.variations) {
        if (variation.signatureToken === undefined || 
            ...
            variation.signedAt === undefined ||
            variation.signedBy === undefined) {
          console.log(`Verification failed for variation ${variation._id} in project ${project._id}`);
          verificationIssues++;
        }
      }
    }
    ...
    if (verificationIssues > 0) {
      console.log(`WARNING: ${verificationIssues} variations still don't have the signature fields`);
    } else {
