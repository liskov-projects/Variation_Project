...
const fixSpecificProject = async () => {
  try {
    ...
    for (let i = 0; i < updatedProject.variations.length; i++) {
      ...
      console.log(
        `  - signatureToken: ${variation.signatureToken === undefined ? "UNDEFINED" : variation.signatureToken}`
      );
      console.log(
        `  - signatureTokenExpiresAt: ${variation.signatureTokenExpiresAt === undefined ? "UNDEFINED" : variation.signatureTokenExpiresAt}`
      );
      console.log(
        `  - signatureData: ${variation.signatureData === undefined ? "UNDEFINED" : variation.signatureData}`
      );
      console.log(
        `  - signedAt: ${variation.signedAt === undefined ? "UNDEFINED" : variation.signedAt}`
      );
      console.log(
        `  - signedBy: ${variation.signedBy === undefined ? "UNDEFINED" : variation.signedBy}`
      );
    }
